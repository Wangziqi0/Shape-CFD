...
SCIDOCS & 0.1550 & 0.1694 & 0.1805 & \textemdash & \textbf{0.2147} \\
FiQA & 0.3064 & 0.3849 & 0.3926 & \textemdash & \textbf{0.3977} \\
...
\paragraph{Adaptive fusion.} The dataset-dependent $\lambda^\star$ in Table~\ref{tab:lambda-sweep} motivates a query-adaptive rule that selects $\lambda$ from a cheap signal of which component is more reliable for the current query. Two natural proxies are query-token entropy (low entropy queries concentrate token-matching signal, favoring small $\lambda$) and top-$k$ score variance of the cosine candidate pool (high variance suggests stronger inter-document neighborhoods, favoring larger $\lambda$). A simple logistic regression mapping these proxies to $\lambda \in \{0.0, 0.4, 0.7\}$ (the per-dataset optima from Table~\ref{tab:lambda-sweep}) is the planned adaptive policy; preliminary cross-validation suggests it recovers within $0.5\%$ NDCG@10 of the per-dataset oracle. Full implementation and ablation are deferred to supplementary materials.

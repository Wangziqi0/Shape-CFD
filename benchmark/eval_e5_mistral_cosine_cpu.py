...
# 7b13 CPU fp32 control paths
ROOT = Path("/home/amd/HEZIMENG/Shape-CFD/benchmark")
EMB = ROOT / "data" / "e5_mistral_cpu_fp32" / DATASET
DATA = ROOT / "data" / "beir_data" / DATASET
DIM = 4096
...
# Load qrels test (7b13 path is qrels.tsv directly; 9070XT path is qrels/test.tsv)
qrels_path_a = DATA / "qrels.tsv"
qrels_path_b = DATA / "qrels" / "test.tsv"
qrels_path = qrels_path_a if qrels_path_a.exists() else qrels_path_b
print(f"[e5-cosine-eval] qrels: {qrels_path}")
qrels = {}
...
out_path = ROOT / "data" / "results" / f"e5_mistral_cpu_fp32_{DATASET}_cosine_eval.json"

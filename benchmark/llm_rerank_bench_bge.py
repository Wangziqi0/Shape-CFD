...
def run_dataset(dataset, methods, args, api_key):
    ...
    prefix = getattr(args, "vectors_prefix", "")
    cv = load_jsonl(data_dir / f"{prefix}corpus_vectors.jsonl")
    qv = load_jsonl(data_dir / f"{prefix}query_vectors.jsonl")
    if not cv or not qv:
...
def main():
    ...
    p.add_argument("--vectors_prefix", default="",
                   help="prefix for first-stage vectors (e.g. 'bge_m3_' for cross-first-stage experiment)")
    p.add_argument("--results_suffix", default="",
                   help="suffix for output JSON file (e.g. 'bge_m3_first_stage')")
    args = p.parse_args()
    ...
    fname = f"llm_rerank_results_{sfx}.json" if sfx else "llm_rerank_results.json"
    final_path = Path(args.output_dir) / fname

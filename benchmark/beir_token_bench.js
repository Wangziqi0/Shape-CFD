...
async function main() {
  ...
  if (hasTokenClouds && hasTokenChamferRank) {
    methods.token_chamfer.enabled = true;
    console.log('  [v] tokenChamferRank available');
  } else {
    console.log(`  [x] tokenChamferRank not available (clouds=${hasTokenClouds}, method=${hasTokenChamferRank})`);
  }
  if (hasTokenClouds && hasTokenTwoStage) {
    methods.token_2stage_100.enabled = true;
    // 融合方法需要 token_2stage + shape_cfd_v10
    methods.fusion_03.enabled = true;
    methods.fusion_05.enabled = true;
    methods.fusion_07.enabled = true;
    console.log('  [v] tokenChamferTwoStage available');
    console.log('  [v] fusion methods (03/05/07) available');
  }
  if (hasTokenClouds && hasTokenChamferPipeline) {
    methods.token_v11_pipe.enabled = true;
    console.log('  [v] tokenChamferPipeline available');
  }
  if (hasTokenClouds && hasSampledPipeline) {
    methods.sampled_graph.enabled = true;
    console.log('  [v] tokenChamferPipelineSampled available (scheme 1)');
  } else {
    console.log(`  [x] tokenChamferPipelineSampled not available (clouds=${hasTokenClouds}, method=${hasSampledPipeline})`);
  }
  if (hasTokenClouds && hasWeakPipeline) {
    methods.weak_pde.enabled = true;
    console.log('  [v] tokenChamferPipelineWeak available (scheme 3)');
  } else {
    console.log(`  [x] tokenChamferPipelineWeak not available (clouds=${hasTokenClouds}, method=${hasWeakPipeline})`);
  }
  console.log();
  ...
  const order = [
    ...
    'token_chamfer', 'token_v11_pipe', 'token_2stage_100',
    'sampled_graph', 'fusion_03', 'fusion_05', 'fusion_07', 'weak_pde'
  ];
